(ns com.grzm.ex.assertions.self-host-test-runner
  ...
  (let [paths-to-provides
        (map (fn [[_ path provides]]
               [path (map second
                          (re-seq #"'(.*?)'" provides))])
             (re-seq #"\ngoog\.addDependency\('(.*)', \[(.*?)\].*"
                     (node-read-file-sync (str output-dir "/goog/deps.js"))))]
    (into {}
          (for [[path provides] paths-to-provides
                provide         provides]
            [(symbol provide) (str output-dir "/goog/" (second (re-find #"(.*)\.js$" path)))]))))
...
(defn run-tests
  ...
  (let [st (cljs/empty-state)]
    (cljs/load-analysis-cache! st 'cljs.core$macros
                               (parse-edn (node-read-file-sync (str output-dir "/cljs/core$macros.cljc.cache.edn"))))
